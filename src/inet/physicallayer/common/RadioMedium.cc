...
void RadioMedium::removeNonInterferingTransmissions()
{
    ...
    for (std::vector<const ITransmission *>::const_iterator it = transmissions.begin(); it != transmissions.begin() + transmissionIndex; it++) {
        ...
        for (std::vector<const IRadio *>::const_iterator jt = radios.begin(); jt != radios.end(); jt++) {
            ...
            if (arrival) {
                SimpleInterval interval(arrival->getStartTime(), arrival->getEndTime(), (void *)transmission);
...
const std::vector<const IReception *> *RadioMedium::computeInterferingReceptions(const IListening *listening, const std::vector<const ITransmission *> *transmissions) const
{
    ...
    RadioCacheEntry *radioCacheEntry = getRadioCacheEntry(radio);
    std::deque<SimpleInterval *> interferingIntervals = radioCacheEntry->receptionIntervals->query(listening->getStartTime(), listening->getEndTime());
    std::vector<const IReception *> *interferingReceptions = new std::vector<const IReception *>();
    for (std::deque<SimpleInterval *>::const_iterator it = interferingIntervals.begin(); it != interferingIntervals.end(); it++)
        interferingReceptions->push_back(getReception(radio, (ITransmission *)((*it)->value)));
...
const std::vector<const IReception *> *RadioMedium::computeInterferingReceptions(const IReception *reception, const std::vector<const ITransmission *> *transmissions) const
{
    ...
    const ITransmission *transmission = reception->getTransmission();
    RadioCacheEntry *radioCacheEntry = getRadioCacheEntry(radio);
    std::deque<SimpleInterval *> interferingIntervals = radioCacheEntry->receptionIntervals->query(reception->getStartTime(), reception->getEndTime());
    std::vector<const IReception *> *interferingReceptions = new std::vector<const IReception *>();
    for (std::deque<SimpleInterval *>::const_iterator it = interferingIntervals.begin(); it != interferingIntervals.end(); it++) {
        const ITransmission *interferingTransmission = (ITransmission *)((*it)->value);
        if (transmission != interferingTransmission)
...
void RadioMedium::addTransmission(const IRadio *transmitterRadio, const ITransmission *transmission)
{
    ...
    for (std::vector<const IRadio *>::const_iterator it = radios.begin(); it != radios.end(); it++) {
        ...
        if (receiverRadio != transmitterRadio) {
            ...
            radioCacheEntry->receptionIntervals->insert(new SimpleInterval(arrival->getStartTime(), arrival->getEndTime(), (void *)transmission));
        }
    }
