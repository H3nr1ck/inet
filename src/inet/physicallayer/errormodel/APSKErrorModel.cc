...
double APSKErrorModel::computePacketErrorRate(const ISNIR *snir) const
{
    ...
    if (bitErrorRate == 0.0)
        ...
        packetErrorRate = 1.0 - pow(1.0 - bitErrorRate, flatTransmission->getPayloadBitLength());
    }
...
double APSKErrorModel::computeBitErrorRate(const ISNIR *snir) const
{
    ...
    double bitErrorRate = modulation->calculateBER(minSNIR, flatTransmission->getBandwidth().get(), flatTransmission->getBitrate().get());
